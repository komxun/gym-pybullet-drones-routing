...
if __name__ == "__main__":
    ...
    parser.add_argument('--algo',        default='IQL',              type=str,             choices=['cc'],                                     help='MARL approach (default: cc)', metavar='')
    ...
    if ARGS.act in [ActionType.AUTOROUTING]:
        ACTION_VEC_SIZE = 1
    else:
    ...
    policies_dict = {"pol"+str(i): (None, observer_space, action_space, {"agent_id": i}) for i in range(ARGS.num_drones)}
    print(f"policy dict is {policies_dict}")
    config["multiagent"] = { 
