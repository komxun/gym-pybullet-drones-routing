...
class ExtendedMARLAviary2(RoutingAviary):
    ...
    def _actionSpace(self):
        ...
        if self.ACT_TYPE == ActionType.AUTOROUTING:
            action_size = 1
            act_lower_bound = np.array([0*np.ones(action_size) for i in range(self.NUM_DRONES)])
            act_upper_bound = np.array([11*np.ones(action_size) for i in range(self.NUM_DRONES)])
            for _ in range(self.ACTION_BUFFER_SIZE):
                self.action_buffer.append(np.zeros((1,action_size)))
            return spaces.Box(low=act_lower_bound, high=act_upper_bound, dtype=np.int8) # 5 discrete actions, details in _preprocessAction()
        else:
    ...
    def _computeObs(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            ret = ret.reshape(1, ret.shape[1]*ret.shape[0]).astype('float32')
            ...
            print(f"Komsun --ret_shape = {ret.shape}")
