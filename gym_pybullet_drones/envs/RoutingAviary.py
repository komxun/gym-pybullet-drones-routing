...
class RoutingAviary(BaseAviary):
    ...
    def __init__(self, 
        ...
        #### Override initial poses #####################################
        alt = 1  # m
        # self.INIT_XYZS = np.vstack([np.array([x*20*self.L for x in range(self.NUM_DRONES)]), \
        #                             np.array([y*4*self.L for y in range(self.NUM_DRONES)]), \
        #                             np.ones(self.NUM_DRONES) * (alt)]).transpose().reshape(self.NUM_DRONES, 3)
    ...
    def _addObstacles(self):
        ...
        if scene == 0:
            ...
            RoutingAviary.OBSTACLE_IDS.add(id1)
            ...
            id2 = p.loadURDF("sphere2.urdf", 
                        ...
                        useFixedBase = False, 
                        physicsClientId=self.CLIENT)
            RoutingAviary.OBSTACLE_IDS.add(id2)
            ...
            id3 = p.loadURDF("sphere2.urdf", 
                        ...
                        useFixedBase = False,
                        physicsClientId=self.CLIENT)
            RoutingAviary.OBSTACLE_IDS.add(id3) 
